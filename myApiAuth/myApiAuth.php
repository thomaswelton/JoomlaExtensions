		...
		if(isset($options['group'])){
      	...
      	if(!is_array($uid)){
			...
			if($result['access_token'] == ''){
				...
				$query = "UPDATE ".$db->nameQuote('#__myapi_users')." SET ".$db->nameQuote('access_token')." = ".$db->quote($facebook->getAccessToken())." WHERE ".$db->nameQuote('uid')." = ".$db->quote($uid);
				$db->setQuery($query);
				$db->query();	
			}
			if($id != ''){      
